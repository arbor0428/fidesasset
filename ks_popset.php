...
if($_SERVER[REMOTE_ADDR] == '106.246.92.237'){
	$sql = "select * from tb_popup where (chk_enable='1' or (chk_enable='2' and sTime <= $SYSTEM_TIME and eTime >= $SYSTEM_TIME)) order by uid desc";
}else{
	$sql = "select * from tb_popup where (chk_enable='1' or (chk_enable='2' and sTime <= $SYSTEM_TIME and eTime >= $SYSTEM_TIME)) and uid!='38' order by uid desc";
}
$result = mysql_query($sql);
$pop_num = mysql_num_rows($result);
...
<div id='divpop<?=$i?>' style="position:absolute; visibility:hidden; z-index:9999999;top:<?=$set_top?>;left:<?=$set_left?>;">
...
<form name='pop<?=$i?>'>
	<tr>
		<td align='center' style="word-break:break-all; border :1 solid #cccccc;">
			<table cellpadding='0' cellspacing='0' border='0' width='<?=$set_width?>' height='<?=$set_height?>' style="TABLE-LAYOUT: fixed;">
				<tr>
					<td align='center'><?=$ment?></td>
				</tr>
				<tr>
					<td height='20'>
						<table cellpadding='0' cellspacing='0' border='0' width='<?=$set_width?>' bgcolor='#161817' height='20'>
							<tr>
								<td width='30' style='padding-left:5px;'><label for='event1'><input type='checkbox' name='event1' id='event1' value='' onclick="closeWin('<?=$D_date?>','<?=$i?>','<?=$reg_date?>');" style='border:0px;'></label></td>
								<td style='padding-top:1px;'><span style="color:#fff;">하루동안 열지않기</span></td>
								<td align='right'><a href="javascript:closeWin('','<?=$i?>','<?=$reg_date?>')"><font color='#ffffff'>[닫기]</font></a></td>
							</tr>
						</table>
					</td>
				</tr>
...
//			pos02 = (screen.height)?(screen.height-h)/2:100;
		...
		var noticeWindow = window.open("ks_winpop.php?uid=<?=$info[uid]?>","<?=$info[reg_date]?>","width="+w+", height="+h+", left="+x+", top="+y+",scrollbars=<?=$scroll?>,toolbar=no,status=no");
