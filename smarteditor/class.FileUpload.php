...
class FileUpload
{
	...
	function FileUpload($directory, $http_file_name, $code='', $limit_size=15, $extension=array('php','cgi','php3','pl','html','htm','exe','jsp')){
	...
	function getValidFileNameEditor($tmp_file_ext){

		if($this->code!=''){
			$str = "1234567890abcdefghijklmopqrstuvwxyz";
			$rcode = substr(str_shuffle($str),0,10);

			$file_name = $this->code.date('ymdHis').$rcode.'.'.$tmp_file_ext;
			$tmp_file_name = $this->code.date('ymdHis').$rcode;
		}
		else {
			$file_name = $this->fileInfo['name'];
			$tmp_file_name = $this->getFileName();
