...
*/       
		...
		if($sz[0] > $sFactor)	$imgW = $sFactor; //임의로 정하는 넓이
		else	$imgW = $sz[0];

		$imgH = $imgW*$sz[1]/$sz[0]; //원본 이미지의 넓이값 대비 높이와 같은 비율로 줄어든 높이값
		if($imgH > $sFactor){$imgH = $sFactor;}
		if($sz[0] < $sz[1]){$imgW = $imgH*$sz[0]/$sz[1];}
