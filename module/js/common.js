...
function isOneCheckModal2(user_price,price,msg,msg2){

	
	if(user_price>price){
		document.getElementById("alertTxt").innerText = msg2;
		$(".GblNotice_open").click();
		return false;	
	}else if(user_price%100000 == 0 && user_price>0){
		return true;
	}else{
		document.getElementById("alertTxt").innerText = msg;
		$(".GblNotice_open").click();
...
function id_pwd_search(){
	...
	var left = (screen.width) ? (screen.width-w)/2 : 0;
    var top = (screen.height) ? (screen.height-h)/2 : 0;
	var win = window.open("/member/find.php","sear","width="+w+", height="+h+", left="+left+", top="+top+", scrollbars=no");
}
...
function onlyNumberModal(event){
	var key = window.event ? event.keyCode : event.which;
	
	if ((event.shiftKey == false) && ((key  > 47 && key  < 58) || (key  > 95 && key  < 106) || key  == 35 || key  == 36 || key  == 37 || key  == 39 || key  == 8  || key  == 46  || key  == 144  || key  == 9   || key  == 190   || key  == 110 )){
		return true;
	} else {
		document.getElementById("alertTxt").innerText = '숫자만 입력이 가능합니다';
		$(".GblNotice_open").click();
		return false;
	}
}
