...
if($total_record != '0'){
	if($total_record > $record_count){
		...
		if($current_page * $record_count > $record_count * $link_count) {
			...
			echo ("<a href=javascript:pageNum2('frm01','$pre_group_start');>≪</a> ");
		}else{
			echo ("<a href=javascript:pageNum2('frm01','$pre_group_start');>≪</a> ");
		}
...
/*
		if($total_page > 1 && ($record_start !=0 )) {
			...
			echo ("<a href=javascript:pageNum2('frm01','$pre_page_start'); class='direction'>Prev</a> ");
		}else{
			echo ("<a href=javascript:pageNum2('frm01','$pre_page_start'); class='direction'>Prev</a> ");
		}
		...
		for($i=0; $i<$link_count; $i++){
			...
			if($input_start < $total_record) {
				if($input_start != $record_start) {
					echo ("<a href=javascript:pageNum2('frm01','$input_start');>$link</a> ");
				} else {
					echo ("<a href=javascript:pageNum2('frm01','$input_start'); class='frst_last this'>$link</a> ");
				}
			}
		}
...
/*
		...
		if($total_page > 1 && ($record_start != ($total_page * $record_count - $record_count))) {
			...
			echo ("<a href=javascript:pageNum2('frm01','$next_page_start'); class='direction'>Next</a> ");
		}else{
...
*/
		...
		if($total_record > (($group + 1) * $record_count * $link_count)) {
			...
			echo("<a href=javascript:pageNum2('frm01','$next_group_start');>≫</a>");
		}else{
