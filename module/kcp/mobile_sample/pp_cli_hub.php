...
<?
    ...
    if ( $req_tx == "pay" )
    {
			...
			$sql = "select sum(amt) as kcpAmt from ks_order_tmp where userip='$cust_ip' and reg_date='$ordr_idxx'";
			$result = mysql_query($sql);
			$row = mysql_fetch_array($result);
			$kcpAmt = $row['kcpAmt'];

            $c_PayPlus->mf_set_ordr_data( "ordr_mony",  $kcpAmt );                                   
...
//    $bSucc = ""; // DB 작업 실패 또는 금액 불일치의 경우 "false" 로 세팅
    ...
    /* = ========================================================================== = */
                ...
                $c_PayPlus->mf_set_modx_data( "mod_desc", "결과 처리 오류 - 취소 요청" );  // 변경 사유
...
?>
   ...
   <form name="pay_info" method="post" action="/orderlist/sub01.php?type=view&uid=<?=$uid?>">
