...
<?
  ...
  if($_SERVER['SERVER_PORT'] == '443')	$url = "https://" . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"];
...
<?
	...
	if($pay_mode == '신용카드'){	
		...
		$ActionResult = 'acnt';
		$pay_method = 'BANK';	//계좌이체
		$pay_mod = 'N';

	}elseif($pay_mode == '가상계좌'){
		$ActionResult = 'vcnt';
		$pay_method = 'VCNT';	//가상계좌
		$pay_mod = 'N';
...
if($callChk){
	...
	//결제정보 임시저장
	$UserOS = 'mobile';
	include '../sample/insert.php';


	if($param_opt_1)	$good_info = $param_opt_1;
}
?>
...
if($errorChk == ''){
?>
<body onload="chk_pay();setTimeout('kcp_AJAX();',300);">
<?
}else{
	echo ("<script language='javascript'>alert('결제취소');location.href='/story/program.php';</script>");
	exit;
}
?>
...
<input type="hidden" name="PayUrl" value="" />
  ...
  <div class="sample">
    ...
    <table class="tbl" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td>
			<select name="ActionResult" onchange="jsf__chk_type();">
				...
				<option value="card" <?if($ActionResult == 'card'){echo 'selected';}?>>신용카드</option>
				<option value="acnt" <?if($ActionResult == 'acnt'){echo 'selected';}?>>계좌이체</option>
				<option value="vcnt" <?if($ActionResult == 'vcnt'){echo 'selected';}?>>가상계좌</option>
      ...
      <tr>
        <th>주문 번호</th>
        <td><input type="text" name="ordr_idxx" class="w200" value="<?=$rTime?>"></td>
      </tr>
      <tr>
        <th>상품명</th>
      ...
      <tr>
        <th>휴대폰번호</th>
        <td><input type="text" name="buyr_tel2" class="w100" value=""></td>
      </tr>
    </table>
...
?>
    ...
    <table class="tbl" cellpadding="0" cellspacing="0">
      <tr>
        <th>수취인이름</th>
        <td><input type="text" name="rcvr_name" class="w100" value="<?=$rcvr_name?>"></td>
      </tr>
      <tr>
        <th>수취인 연락처</th>
        <td><input type="text" name="rcvr_tel1" class="w100" value="<?=$rcvr_tel1?>"></td>
      </tr>
      <tr>
        <th>수취인 휴대폰번호</th>
        <td><input type="text" name="rcvr_tel2" class="w100" value="<?=$rcvr_tel2?>"></td>
      </tr>
      <tr>
        <th>수취인 E-mail</th>
        <td><input type="text" name="rcvr_mail" class="w200" value="<?=$rcvr_mail?>"></td>
      </tr>
      <tr>
        <th>수취인 우편번호</th>
        <td><input type="text" name="rcvr_zipx" class="w100" value="<?=$rcvr_zipx?>"></td>
      </tr>
      <tr>
        <th>수취인 주소</th>
        <td><input type="text" name="rcvr_add1" class="w300" value="<?=$rcvr_add1?>"></td>
      </tr>
      <tr>
        <th>수취인 상세주소</th>
        <td><input type="text" name="rcvr_add2" class="w300" value="<?=$rcvr_add2?>"></td>
      </tr>
    </table>
...
-->
  ...
  <!-- 가상계좌 설정 -->
  <input type="hidden" name="ipgm_date"       value="<?=date('Ymd')?>"/>
  ...
  <input type="hidden" name="param_opt_1"     value="<?=$good_info?>">
...
?>
  ...
  <input type="hidden" name='pay_mod'   value="<?=$pay_mod?>">
  <!-- 배송소요기간 -->
