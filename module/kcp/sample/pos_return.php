...
<?
    ...
    if ( $tx_cd == "TX00" )
    {
		...
		$sql = "select sum(payAmt) as kcpAmt from ks_userClass where rTime='$order_no' and paynum='$tno'";
		...
		if($num){
			...
			if($ipgm_mnyx == $kcpAmt){
				$vaDate = date('Y-m-d H:i:s');
				$vaTime = mktime();

				$sql = "update ks_userClass set payOk='결제확인', vaDate='$vaDate', vaTime='$vaTime' where rTime='$order_no' and paynum='$tno'";
				$result = mysql_query($sql);
		...
		//개별강좌결제
			...
			$sql = "select * from ks_programEtc where rTime='$order_no' and paynum='$tno'";
			$result = mysql_query($sql);
			$num = mysql_num_rows($result);
			...
			if($num){
				...
				if($ipgm_mnyx == $payAmt){
					...
					$sql = "update ks_programEtc set payOk='결제확인', vaDate='$vaDate', vaTime='$vaTime' where rTime='$order_no' and paynum='$tno'";
					$result = mysql_query($sql);
