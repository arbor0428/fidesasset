...
if($type == 'write' || $type == 'edit'){
	...
	if($type == 'write'){
		...
		$sql = "insert into ks_userlist (status,userid,pwd,name,userNum,userOrder,sex,bDate,bTime,userType,car,carNum,zipcode,addr01,addr02,bank,accName,account,email01,email02,phone01,phone01Txt,phone02,phone02Txt,memo,reduction,upfile01,realfile01,upfile02,realfile02,cok,cokPost,cokSms,cokEmail,cokPhone,health,healthBaby,healthEtc,joinType,getDate,getTime,rDate,rTime) values ";
		$sql .= "('$status','$userid','$pwd','$name','$userNum','$userOrder','$sex','$bDate','$bTime','$userType','$car','$carNum','$zipcode','$addr01','$addr02','$bank','$accName','$account','$email01','$email02','$phone01','$phone01Txt','$phone02','$phone02Txt','$memo','$reduction','$arr_new_file[1]','$real_name[1]','$arr_new_file[2]','$real_name[2]','$cok','$cokPost','$cokSms','$cokEmail','$cokPhone','$health','$healthBaby','$healthEtc','$joinType','$getDate','$getTime','$rDate','$rTime')";
		$result = mysql_query($sql);
		...
		if($point01 != $point){
			$sql = "insert into ks_member_point_log (userid,point01,point02,reg_date) values ('$userid','$point01','$point','$editDate')";
			$result = mysql_query($sql);
		}
