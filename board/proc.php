...
switch($type){
	...
	case 'edit' :
		...
		//공연/전시 > 공연정보 or 열린공간 > 공지사항 or 열린공간 > 채용정보
		if($table_id == 'table_1631524937' || $table_id == 'table_1512539542' || $table_id == 'table_1512539551'){
			//일자
			if($sDate){
				$startDate = $sDate;
			...
			//간단설명
			if($sDataTxt){
				$sDataTxt = Util::textareaEncodeing($sDataTxt);
				$sDataTxt = addslashes($sDataTxt);
			}

		//대관시설 > 은평문화예술회관 or 숲속극장
...
//		$ment = iconv('CP949','EUC-KR//IGNORE',$ment);
		...
		if($type=='write'){
			...
			//월간일정일 경우 선택한 기간내 일수에 맞춰 개별등록
			if($table_id == 'table_1631524949'){
				//일자
				if($sDate){
					$startDate = $sDate;
					$sDateTxt = substr($sDate,0,10);
