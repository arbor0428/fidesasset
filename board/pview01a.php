		...
		for($i=1; $i<=$upload_chk; $i++){
			...
			if($uf){
				...
				if($file_exe == 'jpg' || $file_exe == 'jpeg' || $file_exe == 'gif' || $file_exe == 'png' || $file_exe == 'bmp'){
					${'img'.$no} = "<img src='".$boardRoot."upfile/$uf' style='max-width:1000px;'><br>";
				}
			}
		}
...
?>
	<tr> 
		<th width='17%' class='tab_bbs'>평점</th>
		<td width='33%' class='tab'><?=$starImg?></td>
	...
	<tr> 
		<th class='tab_bbs'>작성자</th>
		<td class='tab'><?=$name?></td>
		<th class='tab_bbs'>조회수</th>
		<td class='tab'><?=$hit?></td>
	</tr>
